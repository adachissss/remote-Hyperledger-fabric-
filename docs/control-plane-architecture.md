...
| 可视化 | 当前使用 React 组件、CSS 与 SVG；React Flow/ECharts 作为后续选项 | 先保持依赖精简，再按复杂拓扑和指标规模引入专用库 |
...
flowchart LR
    ...
    GATEWAY --> FABRIC
    SHELL[Direct network.sh] --> SCRIPTS
    SCRIPTS --> DISCOVERY[Discovery manifests / Docker labels]
    API --> DISCOVERY
```
...
| 网络清理/删除 | `network.sh down` + Registry 删除 | 输入网络名二次确认、目标资源核验 |
| 配置与拓扑 | `config/orgs.yaml`、Compose 配置和 Docker inspect | 后端直接解析为结构化只读 API |
...
Web 和 `pfctl` 是同一控制平面的两个正式客户端。二者共享 `packages/shared` 的请求/响应 schema，通过同一 REST/SSE API 使用 Network Registry、managed config builder、端口规划器和 Job Service。`pfctl` 不直接生成工作区，也不绕过数据库互斥；其 YAML/JSON 文件只是创建或导入请求的序列化形式。

直接运行 `network.sh` 时不要求 API 在线。脚本在每次成功的生命周期状态变化后，生成一次不含秘密的发现内容，分别原子替换工作区镜像清单和用户发现索引，并给 Docker Network、Peer、Orderer、CA、CouchDB 与显式 Volume 加归属标签。API 将发现项标为 `unregistered`、`registered`、`conflict` 或 `stale`，只有用户确认后才注册，不会自动执行外部脚本。

网络销毁分为两个明确层级：`down` 清理运行资源但保留注册和工作区，“彻底删除”则在 `down` 成功后删除注册记录、释放端口，并删除 managed workspace。导入网络的外部工作区永远不由控制平面递归删除。`down` 除 Compose 服务外，还按 Compose project、目标 Docker network、Peer 链码容器前缀识别 Fabric 创建的 builder/runtime 容器；只清理目标 namespace，不使用全局 Docker prune。Docker network 仍被占用时作业必须失败，不能用成功状态掩盖残留。
...
`pfctl` 默认同时消费 SSE 和 JSON 历史查询：事件 ID 用于去重和断线重放，Job 查询决定最终状态；短暂的历史/状态 API 故障会有限重试。生命周期命令可用 `--detach` 只返回 Job，跟随模式以成功、失败、取消分别返回 `0`、`1`、`2`，参数或 API 协议错误返回 `3`。

Operations 页面提供四个脚本生命周期入口，以及独立的彻底删除入口、作业取消、历史步骤和日志查看。`down` 与彻底删除都需要输入网络 ID 确认。删除仍作为网络级互斥作业执行；成功后 Job/JobEvent 历史保留，即使网络注册记录已经移除仍可审计。当前操作者记录为本地 `local-user`，认证与细粒度 RBAC 仍属于后续加固阶段。
...
GET    /api/v1/networks
GET    /api/v1/networks/discoveries
POST   /api/v1/networks/discoveries/:discoveryNetworkId/import
