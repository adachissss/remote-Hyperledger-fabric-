...
class RSIdentity extends Contract {
    ...
    async GetEnterpriseUsers(ctx) {
        ...
        let res = await iterator.next();
        while (!res.done) {
            const enterprise = JSON.parse(res.value.value.toString());
        ...
        const enterprise = enterprises[0]; // 假设一个管理员只管理一个企业
        ...
        const userIterator = await ctx.stub.getQueryResult(JSON.stringify(userQuery));
        const users = [];
    ...
    async help(ctx) {
        const info = {
            ...
            version: "1.1.0 (Added History Support)",
            description: "负责身份管理、权限控制、信誉分计算与合规性检查及溯源",
            interfaces: [
